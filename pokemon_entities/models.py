...
class Entities(models.Model):
    ...
    appearted_at = models.DateTimeField(verbose_name="Момент явления", blank=True, null=True)
    disappearted_at = models.DateTimeField(verbose_name="Момент исчезновения", blank=True, null=True)
    ...
    def __str__(self):
        date_in = self.appearted_at.date() if self.appearted_at else "always before"
        date_out = self.disappearted_at.date() if self.disappearted_at else "and forever"
        title_text = '(always)' if not self.appearted_at and not self.disappearted_at else f'({date_in} - {date_out})'
